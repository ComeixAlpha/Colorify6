...
class KDTree {
  ...
  double Function(PaletteEntry?, PaletteEntry?) distance;

  KDTree(List<PaletteEntry> palette, this.distance) : axisOrder = _findBestAxisOrder(palette) {
  ...
  KDNode? _findNearest(KDNode? node, PaletteEntry target, int depth, KDNode? best) {
    ...
    if (best == null || distance(target, node.entry) < distance(target, best.entry)) {
      nextBest = node;
    } else {
      nextBest = best;
    }
    ...
    if (nextBranch == null || _distanceToPlane(target, node.entry, axis) < distance(target, nextBest?.entry)) {
      if (target.toList()[axis] < node.entry.toList()[axis]) {
        nextBest = _findNearest(node.right, target, depth + 1, nextBest);
      } else {
        nextBest = _findNearest(node.left, target, depth + 1, nextBest);
